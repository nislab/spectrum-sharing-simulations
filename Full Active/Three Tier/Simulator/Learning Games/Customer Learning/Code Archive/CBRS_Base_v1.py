...
SIM_TIME = 2*(10**6)/LAMc # length of time to run simulation over; scales so that ~5,000,000 customers are generated in each round
T_START = 0.05*SIM_TIME # time to begin collecting statistics to allow system to reach steady state
C = 0.486111 # Cost to join Premium class
...
class Channel:
	...
	def incumbents(self, env):
		...
		while True:
			# randomized Poission interarrival rate
			yield env.timeout(self.generator.exponential(1/LAMi))
...
def main():
	...
	PHI = 0.5 # Initial Belief, is updated via loop
	'''
	Main Simulator Loop
	'''
