...
SCALE = 1.3692
SHAPE = -0.2669
MU = 0.627845 # resulting service rate, used for expected value computation
RHO = LAM/MU # resulting traffic rate, used for expected value computation
K = 1.30018 # resulting 2nd moment, used for expected value computation
'''
...
C = 326.82 # Cost to join premium queue
...
def Simulator(lam, loc, scale, shape, phi, lami, mui, ki):
	...
	"""
	SIM_TIME = (10**5)/LAM # length of time to run simulation over; scales so that 1,000,000 users are generated
...
PHI = 0.999 # Initial value of PHI
# write to file
...
for k in range(ITERATIONS):
	...
	# use placeholder values if phi = 0 or 1, as otherwise there are 0 custoemrs in other class
	if PHI == 0:
