...
def main():
	...
	for i in range(ITERATIONS):
		...
		if i < ITERATIONS/2:
			# customer statistics
			LAMc = 0.8 # Arrival rate 
			MUc = 1 # Service rate = 1/(first moment of service)
			RHOc = LAMc/MUc # traffic load, used for validation
			Kc = 2  # Service Distribution; defined such that second moment of service = K/MU^2
		...
		if i < 7*ITERATIONS/10:
			C = 28 # actual value 35.7427, 28 is approximately phi=0.95 cost for original customer params
		else:
			C = 0.10 # actual value 0.139137, 0.10 is < phi = 0.95 cost for orginal customer params
