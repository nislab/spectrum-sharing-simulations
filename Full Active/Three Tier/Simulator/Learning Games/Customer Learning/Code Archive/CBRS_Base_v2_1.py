...
def main():
	...
	for i in range(ITERATIONS):
		...
		if i < ITERATIONS/5:
			LAMi = 0.1 # Arrival rate 
			MUi = 2 # Service rate = 1/(first moment of service)
			...
			MUi = 2 # Service rate = 1/(first moment of service)
			RHOi = LAMi/MUi # traffic load, used for validation
			Ki = 1 # Service Distribution; defined such that second moment of service = K/MU^2
		if Ki > 1:
		...
		with open(resultout,'a') as file:
			...
			writer.writerow([PHI_last,mean_wait_p,expected_p,mean_wait_g,expected_g])
		file.close()
