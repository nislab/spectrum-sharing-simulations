...
SIM_TIME = (10**5)/LAMc # length of time to run simulation over; scales so that ~100,000 customers are generated in each round
...
ITERATIONS = 30 # number of rounds to run the game over
...
class Channel:
	def __init__(self, env, phi,LAMi,MUi,Ki,SHAPEi,SCALEi):
	...
	def customers(self, env):
		...
		while True:
			...
			yield env.timeout(self.generator.exponential(1/LAMc))
			'''
...
def main():
	...
	PHI = 0.95 # Initial Belief, is updated via loop
	'''
	Main Simulator Loop
	'''
	for i in range(ITERATIONS):
		...
		if i < 3*ITERATIONS/5:
			C = 1.41 # actual C(1) = 1.4167
		else:
			C = 0.14 # actual C(1) = 0.1464
		...
		CBRS = Channel(env,PHI,LAMi,MUi,Ki,SHAPEi,SCALEi)
		env.run(until=SIM_TIME)
		# Record statistics, including mean wait time per class
		
		if PHI < 0.5 or PHI > 0.95:
			# if all (or almost all) customers in one class, use expected values
			mean_wait_p = 1/(MUc*(1-RHOi)) + (Ki*RHOi/MUi+PHI*Kc*RHOc/MUc)/(2*(1-RHOi)*(1-(RHOi+PHI*RHOc))) # mean_wait of priority customers
			mean_wait_g = 1/(MUc*(1-(RHOi+PHI*RHOc))) + (Ki*RHOi/MUi+Kc*RHOc/MUc)/(2*(1-(RHOi+PHI*RHOc))*(1-(RHOi+RHOc))) # mean_wait of general custoemrs
		else:
