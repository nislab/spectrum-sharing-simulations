...
def Simulator(LAM, MU, PHI, SIGMA, costfile):
    ...
    class SimEnv:
        def __init__(self, env):
            ...
            #self.setup = True # flag to trigger setup for SUs
        ...
        def server(self, env):
            ...
            while True:
                if self.q.empty():
                    ...
                    self.idle = True
                    #self.setup = True # reset flag on idle period
                ...
                if (self.next[0] == 0):
                    ...
                    #secondary user
                    try:
                        # setup time, only on first user in SU service period
                        inSetup = True
                        ...
                        #self.setup = True # reset flag on premption
                        # current job interrupted by higher priority arrival, reinsert into queue with time remaining updated
                        if inSetup:
                            self.q.push(self.next[0],self.next[1], self.next[2], self.next[3]) # do not decrement service times
                        else:
