        ...
        def server(self, env):
            ...
            while True:
                if self.q.empty():
                    ...
                    yield env.timeout(np.random.exponential(SIGMA)) # first arrival after idle period incurrs additional setup
                else:
                    # queue not empty, immediately get next customer
                ...
                self.idle = False
                start = env.now
                # if beyond T_START, and is user's intial entry, record wait time
                if (env.now > T_START and self.next[2] == self.next[3]):
