...
def Simulator(LAM, MU, PHI, SIGMA, costfile):
    ...
    class SimEnv:
        def __init__(self, env):
            ...
            self.PUperiod = False # flag to mark PU busy period
            self.PUcount = 0 # count of primary users currently present
        ...
        def arrivals(self, env):
            ...
            while True:
                ...
                # otherwise, if in middle of setup period and PU arrives, trigger interuption    
                elif(not self.PUperiod and priority == 0):
                    self.server_proc.interrupt()
        ...
        def server(self, env):
            ...
            while True:
                ...
                '''
                Check flag for whether currently in PU busy period - if start of new PU busy period, reset flag accordingly
                '''
                ...
                try:
                    ...
                    if (env.now > T_START):
                        self.total_w[self.next[0]] += (env.now - self.next[1]) # total wait time is exit time - initial queue entry - service time
                ...
                if (self.PUperiod and self.PUcount == 0):
                    # end of PU busy period, run setup; subject to interrupt
                    try: 
                        self.PUperiod = False
