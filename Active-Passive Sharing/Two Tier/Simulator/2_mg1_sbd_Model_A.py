...
LAM = [0.2,0.3,0.4] # Arrival rates of customers

NUMLAM = len(LAM)
MU = 1 # Service rate of customers; defined as 1 over first moment of service

# Define parameters of server breakdowns
LAMBDA_IN = 1 # (exponential) rate at which server breaks down
MU_IN = 1 #rate at which server gets repaired
...
ITERATIONS = 5 # number of independent simulations
ALPHA = 0.05 # confidence interval is 100*(1-alpha) percent


K = 1# float(sys.argv[4]) # Service Distribution; defined such that second moment of service is K over MU^2
if K < 1:
...
K_IN = 1# float(sys.argv[5]) # Service Repair Distribution; defined such that second moment of service is K_IN over MU_IN^2
if K_IN < 1:
...
def arrivals_PU(env, server, rate, t_start):
    off_time =0
    while True:
        on_time = np.random.exponential(1/rate)
        yield env.timeout(off_time+on_time) # general off time + exponential on time 
        arrival = env.now # mark arrival time
        priority = 0 # PU arrival
        if K_IN == 1: 
            serv_time = 1/MU_IN # Special case for Deterministic system
        else:
            serv_time = np.random.gamma(SHAPE_IN,SCALE_IN)
        off_time = serv_time
        env.process(provider(env,arrival,priority,serv_time,t_start,server))   
...
plt.plot(LAM,NPAnalytical_Wait_High, label='High Class, Analytical') # Plot of Expected Wait (Repair) Times, class 0
plt.errorbar(LAM, Sample_Wait[:,1], yerr=Error[:,1], fmt='x', label='Low Class, Simulated') # Plot of Simulated Wait Times, class 1
plt.errorbar(LAM, Sample_Wait[:,0], yerr=Error[:,0], fmt='x', label='High Class, Simulated') # Plot of Simulated Wait (Repair) Times, class 0
plt.title('Comparison of Analytical results to simulation outputs (K=%d, MU=%.3f)' %(K, MU))
plt.xlabel('Lambda')
plt.ylabel('Mean System Wait/Repair Time')
plt.legend()
plt.show()
